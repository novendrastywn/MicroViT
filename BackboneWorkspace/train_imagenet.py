...
group.add_argument('--weight-decay', type=float, default=0.025,
                    help='weight decay (default: 0.05)')
group.add_argument('--clip-grad', type=float, default=0.02, metavar='NORM',
                    help='Clip gradient norm (default: None, no clipping)')
group.add_argument('--clip-mode', type=str, default='agc',
...
group.add_argument('--train-interpolation', type=str, default='bicubic',
...
group.add_argument('--eval-metric', default='acc1', type=str, metavar='EVAL_METRIC',
...
group = parser.add_argument_group('Weights and Biases Logger')
group.add_argument('--enable_wandb', action='store_true', default=False,
                        help="enable logging to Weights and Biases")
group.add_argument('--project', default='RepFormer', type=str,
...
def main():
    ...
    inputs = torch.randn(1, 3, args.input_size[1], args.input_size[2], device='cpu')
    n_parameters = round(parameter_count(model.eval())[""]/1e6, 4)
    n_flops = round(FlopCountAnalysis(model, inputs).total() / 1e9, 4)
    model.train()
    print(model)
    ...
    if args.local_rank == 0:
        _logger.info(
            f'Model {safe_model_name(args.model)} created, param count:{n_parameters}, gflops:{n_flops}')
    ...
    if args.auto_resume:
        ...
        output_dir = os.path.join(args.output, exp_name) if args.output else os.path.join('./OUTPUT/train', exp_name)
    ...
    loader_train = create_loader(
        ...
        num_aug_repeats=args.world_size, #args.aug_repeats
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(
                ...
                wandb_logger=wandb_logger, start_steps=epoch * num_training_steps_per_epoch, max_acc1=max_accuracy
                )
            ...
            if max_accuracy < eval_metrics["acc1"]:
                max_accuracy = eval_metrics["acc1"]
            ...
            #         epoch, model_ema.module, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, log_suffix=' (EMA)')
            #     # eval_metrics = ema_eval_metrics
            #     eraseTerminal(1)

            #     if max_accuracy_ema < ema_eval_metrics["acc1"]:
            #         max_accuracy_ema = ema_eval_metrics["acc1"]

            #     log_stats.update({**{f'test_{k}_ema': v for k, v in ema_eval_metrics.items()}})
            #     log_stats.update({'max_ema_acc1': max_accuracy_ema})
            ...
            if output_dir is not None:
                utils.update_summary(
                    ...
                    write_header=best_metric is None, log_wandb=False and has_wandb)

            if saver is not None:
                # save proper checkpoint with eval metric
                save_metric = eval_metrics[eval_metric]
                best_metric, best_epoch = saver.my_save_checkpoint(epoch, num_epochs, metric=save_metric)
            
            if wandb_logger:
                wandb_logger.log_epoch_metrics(log_stats)
            
            # if epoch%5==0:
            #     if saver is not None and best_metric is not None:
            #         eraseTerminal(1)
            #         _logger.info('Test: epoch [{0:>4d}/{1:>4d}] '
            #                     'Loss:{2:>7.4f} '
            #                     'Top-1, Top-5 :{3:>7.4f}, {4:>7.4f} '
            #                     '** Best Top-1: {5:>7.4f} (epoch {6:>4d})'.format(
            #                         epoch, num_epochs, 
            #                         eval_metrics['loss'],
            #                         eval_metrics['acc1'], eval_metrics['acc5'],
            #                         best_metric, best_epoch))
            #     else:
            #         eraseTerminal(1)
            #         _logger.info('Test: epoch [{0:>4d}/{1:>4d}] '
            #                     'Loss:{2:>7.4f} '
            #                     'Top-1, Top-5 :{3:>7.4f}, {4:>7.4f}'.format(
            #                         epoch, num_epochs, 
            #                         eval_metrics['loss'],
            #                         eval_metrics['acc1'], eval_metrics['acc5']))
...
def train_one_epoch(
        ...
        wandb_logger=None, start_steps=None, max_acc1=0):
    ...
    for batch_idx, (input, target) in pbar:
        ...
        pbar.set_description(
                c_print('Train [{0}] => '
                    ...
                    'Max_Acc1 [{epoch_1}]: {max_acc1:.2f}'.format(
                        epoch,
                        loss=losses_m,
                        batch_time=batch_time_m,
                        rate=input.size(0) * args.world_size / batch_time_m.val,
        ...
        for batch_idx, (input, target) in pbar:
            ...
            pbar.set_description(
                c_print('{0} [{1}]:  '
                    ...
                    'Acc@1: {top1.val:>7.4f} ({top1.avg:>7.4f})  '
                    'Acc@5: {top5.val:>7.4f} ({top5.avg:>7.4f})'.format(
                        log_name, epoch, batch_time=batch_time_m,
                        loss=losses_m, top1=top1_m, top5=top5_m),
                        color=(20, 250, 10)))
...
class CheckpointSaver:
    ...
    def my_save_checkpoint(self, epoch, num_epochs, metric=None):
        assert epoch >= 0
